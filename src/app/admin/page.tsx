...
export default function AdminPage() {
  ...
  const [lodgingFilter, setLodgingFilter] = useState("전체");
  const [movementFilter, setMovementFilter] = useState("전체");
  const [notice, setNotice] = useState("");
  const inquiryDrawerRef = useRef<HTMLElement>(null);
  ...
  const filteredInquiries = useMemo(() => inquiries.filter((item) => {
    const statusLabel = item.status === "answered" ? "답변 완료" : item.status === "closed" ? "종료" : "답변 대기";
    const text = `${item.code} ${item.category} ${item.title}`.toLowerCase();
    return (inquiryStatus === "전체" || statusLabel === inquiryStatus) && text.includes(inquiryQuery.toLowerCase());
  }), [inquiries, inquiryQuery, inquiryStatus]);

  const filteredLogistics = useMemo(() => logistics.filter((item) => {
    const text = `${item.name} ${item.organization} ${item.note}`.toLowerCase();
    return (lodgingFilter === "전체" || item.lodgingNeeded === lodgingFilter)
      && (movementFilter === "전체" || item.outboundMethod === movementFilter)
      && text.includes(logisticsQuery.toLowerCase());
  }), [logistics, logisticsQuery, lodgingFilter, movementFilter]);

  useEffect(() => {
    if (!selectedInquiry) return;
    const previousOverflow = document.body.style.overflow;
    document.body.style.overflow = "hidden";
    const drawer = inquiryDrawerRef.current;
    const focusable = drawer?.querySelectorAll<HTMLElement>('button, textarea, input, select, a[href], [tabindex]:not([tabindex="-1"])');
    (drawer?.querySelector<HTMLElement>("textarea") || drawer?.querySelector<HTMLElement>("h2") || focusable?.[0])?.focus();
  ...
  async function replyToInquiry(event: React.FormEvent) {
    ...
    setAdminReply(""); setSelectedInquiry(null); setNotice("문의 답변을 저장했습니다."); await loadData();
  ...
  if (status === "login") return <main className="admin-login"><a className="admin-home" href="/">← 홈페이지</a><form className="admin-login-card" onSubmit={login}><img src="/images/daedong-logo.png" alt="DAEDONG" /><p className="eyebrow">ADMIN ACCESS</p><h1>교육 운영 대시보드</h1><p>지정된 관리자 계정으로 로그인해주세요.</p><label>관리자 아이디<input type="text" inputMode="numeric" autoComplete="username" value={credentials.email} onChange={(event) => setCredentials({ ...credentials, email: event.target.value })} required /></label><label>비밀번호<input type="password" autoComplete="current-password" value={credentials.password} onChange={(event) => setCredentials({ ...credentials, password: event.target.value })} required /></label>{error && <p className="form-error">{error}</p>}<button type="submit">관리자 로그인 →</button></form></main>;
  ...
  return <main className="admin-page">
    ...
    <div className="admin-main">
      ...
      <section className="admin-section" id="inquiries"><div className="admin-section-title"><div><p>INQUIRY</p><h2>문의함</h2></div><span>{filteredInquiries.length}건</span></div><div className="table-tools"><input value={inquiryQuery} onChange={(event) => setInquiryQuery(event.target.value)} placeholder="문의번호·유형·제목 검색" aria-label="문의 검색" /><select value={inquiryStatus} onChange={(event) => setInquiryStatus(event.target.value)} aria-label="문의 상태 필터">{["전체", "답변 대기", "답변 완료", "종료"].map((item) => <option key={item}>{item}</option>)}</select></div><div className="admin-table-wrap"><table><thead><tr><th>최근 업데이트</th><th>문의 번호</th><th>유형</th><th>제목</th><th>상태</th><th /></tr></thead><tbody>{filteredInquiries.map((item) => <tr key={item.id}><td>{new Date(item.updatedAt).toLocaleString("ko-KR")}</td><td><strong>{item.code}</strong></td><td>{item.category}</td><td title={item.title}>{item.title}</td><td><span className={`inquiry-admin-status ${item.status}`}>{item.status === "answered" ? "답변 완료" : item.status === "closed" ? "종료" : "답변 대기"}</span></td><td><button onClick={(event) => { inquiryTriggerRef.current = event.currentTarget; setSelectedInquiry(item); setAdminReply(""); }}>대화 보기 →</button></td></tr>)}</tbody></table><div className="admin-mobile-list">{filteredInquiries.map((item) => <article key={item.id}><div><strong>{item.code}</strong><span className={`inquiry-admin-status ${item.status}`}>{item.status === "answered" ? "답변 완료" : item.status === "closed" ? "종료" : "답변 대기"}</span></div><p>{item.category} · {new Date(item.updatedAt).toLocaleDateString("ko-KR")}</p><h3>{item.title}</h3><button onClick={(event) => { inquiryTriggerRef.current = event.currentTarget; setSelectedInquiry(item); setAdminReply(""); }}>대화 보기 →</button></article>)}</div>{filteredInquiries.length === 0 && <div className="empty-state">조건에 맞는 문의가 없습니다.</div>}</div></section>
      <section className="admin-section" id="logistics"><div className="admin-section-title"><div><p>TRAVEL & STAY</p><h2>숙박·이동 응답</h2></div><a className="admin-export-link" href="/api/admin/export?type=logistics">응답 다운로드 ↓</a></div><div className="table-tools logistics-tools"><input value={logisticsQuery} onChange={(event) => setLogisticsQuery(event.target.value)} placeholder="이름·소속·전달사항 검색" aria-label="숙박 이동 응답 검색" /><select value={lodgingFilter} onChange={(event) => setLodgingFilter(event.target.value)} aria-label="숙박 여부 필터">{["전체", "숙박 필요", "숙박 불필요"].map((item) => <option key={item}>{item}</option>)}</select><select value={movementFilter} onChange={(event) => setMovementFilter(event.target.value)} aria-label="이동 방법 필터">{["전체", "셔틀버스 이동", "자차 이동"].map((item) => <option key={item}>{item}</option>)}</select></div><div className="admin-table-wrap"><table><thead><tr><th>제출일</th><th>이름</th><th>소속</th><th>숙박</th><th>이동 방법</th><th>복귀</th><th>전달사항</th></tr></thead><tbody>{filteredLogistics.map((item) => <tr key={item.id}><td>{new Date(item.submittedAt).toLocaleString("ko-KR")}</td><td><strong>{item.name}</strong></td><td>{item.organization}</td><td>{item.lodgingNeeded}</td><td>{item.outboundMethod}</td><td>{item.returnMethod}</td><td className="note-cell" title={item.note || ""}>{item.note || "-"}</td></tr>)}</tbody></table><div className="admin-mobile-list">{filteredLogistics.map((item) => <article key={item.id}><div><strong>{item.name}</strong><span>{item.lodgingNeeded}</span></div><p>{item.organization} · {new Date(item.submittedAt).toLocaleDateString("ko-KR")}</p><dl><div><dt>이동</dt><dd>{item.outboundMethod}</dd></div><div><dt>복귀</dt><dd>{item.returnMethod}</dd></div>{item.note && <div><dt>전달</dt><dd>{item.note}</dd></div>}</dl></article>)}</div>{filteredLogistics.length === 0 && <div className="empty-state">조건에 맞는 응답이 없습니다.</div>}</div></section>
    ...
    {selectedInquiry && <div className="admin-drawer-backdrop" onClick={() => setSelectedInquiry(null)}><aside ref={inquiryDrawerRef} className="admin-drawer inquiry-admin-drawer" role="dialog" aria-modal="true" aria-labelledby="inquiry-drawer-title" onClick={(event) => event.stopPropagation()}><button className="drawer-close" onClick={() => setSelectedInquiry(null)} aria-label="문의 대화 닫기">×</button><p className="eyebrow">PRIVATE INQUIRY · {selectedInquiry.code}</p><h2 id="inquiry-drawer-title" tabIndex={-1}>{selectedInquiry.title}</h2><div className="detail-meta"><span>{selectedInquiry.category}</span><span>{selectedInquiry.status === "answered" ? "답변 완료" : selectedInquiry.status === "closed" ? "종료" : "답변 대기"}</span></div><div className="admin-conversation">{selectedInquiry.messages.map((item) => <article className={item.sender} key={item.id}><div><strong>{item.sender === "admin" ? "관리자" : "익명 작성자"}</strong><time>{new Date(item.createdAt).toLocaleString("ko-KR")}</time></div><p>{item.content}</p></article>)}</div>{selectedInquiry.status !== "closed" && <form className="admin-inquiry-reply" onSubmit={replyToInquiry}><label>관리자 답변<textarea value={adminReply} maxLength={3000} onChange={(event) => setAdminReply(event.target.value)} required /></label><button type="submit">답변 저장 및 전송</button></form>}<div className="admin-inquiry-actions">{selectedInquiry.status === "closed" ? <button onClick={() => void changeInquiryStatus("open")}>문의 다시 열기</button> : <button onClick={() => void changeInquiryStatus("closed")}>문의 종료</button>}</div><div className="admin-danger-zone"><strong>위험 영역</strong><p>삭제한 문의와 대화는 복구할 수 없습니다.</p><button className="delete-inquiry" onClick={() => void removeInquiry()}>문의 삭제</button></div></aside></div>}
